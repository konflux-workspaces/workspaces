...
var _ = Describe("List", func() {
	...
	DescribeTable("workspace GET handler: read",
		func(
			mapperFunc workspace.ListWorkspaceMapperFunc,
			readHandler workspace.ListWorkspaceQueryHandlerFunc,
			marshaler marshal.MarshalerProvider,
			responseFunc func() http.ResponseWriter,
		) {
			response := responseFunc()
			handler := workspace.NewListWorkspaceHandler(mapperFunc, readHandler, marshaler)
			handler.ServeHTTP(response, request)
		},
		Entry("failure in marshal provider", workspace.MapListWorkspaceHttp, nopListHandler, errorMarshalProvider, func() http.ResponseWriter {
			fake.EXPECT().WriteHeader(http.StatusBadRequest)
			return fake
		}),
		Entry("failure in read handler", workspace.MapListWorkspaceHttp, badListHandler, marshal.DefaultMarshalerProvider, func() http.ResponseWriter {
			fake.EXPECT().WriteHeader(http.StatusInternalServerError)
			return fake
		}),
		Entry("failure marshaling response", workspace.MapListWorkspaceHttp, nopListHandler, badMarshalProvider, func() http.ResponseWriter {
			fake.EXPECT().WriteHeader(http.StatusInternalServerError)
			return fake
		}),
		Entry("failure to write response", workspace.MapListWorkspaceHttp, nopListHandler, marshal.DefaultMarshalerProvider, func() http.ResponseWriter {
...
func badListHandler(ctx context.Context, cmd coreworkspace.ListWorkspaceQuery) (*coreworkspace.ListWorkspaceResponse, error) {
	return nil, fmt.Errorf("bad create handler")
}

func nopListHandler(_ctx context.Context, cmd coreworkspace.ListWorkspaceQuery) (*coreworkspace.ListWorkspaceResponse, error) {
	return &coreworkspace.ListWorkspaceResponse{
		Workspaces: restworkspacesv1alpha1.WorkspaceList{},
	}, nil
}

func buildListRequest(workspace *restworkspacesv1alpha1.Workspace) *http.Request {
